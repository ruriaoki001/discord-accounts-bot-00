...
const { Client: Client_ } = require('discord.js-selfbot-v13');

console.log('🚀 Starting Discord Alt Online Manager...');
...
class StatusGenerator {
  constructor() {
    this.activities = {
      PLAYING: [
        ...
        'Call of Duty', 'Cyberpunk 2077', 'Elden Ring', 'GTA V', 'Rocket League',
        'Among Us', 'Fall Guys', 'Stardew Valley', 'Terraria', 'Skyrim',
        'The Witcher 3', 'Red Dead Redemption 2', 'Overwatch 2', 'CS:GO', 'Dota 2',
        'Hades', 'Hollow Knight', 'Rust', 'ARK Survival', 'Roblox', 'PUBG',
        'VS Code', 'Photoshop', 'Blender', 'Unity', 'Unreal Engine', 'IntelliJ IDEA',
        'Terminal', 'Docker', 'Visual Studio', 'Premiere Pro', 'After Effects', 'discord.gg/'
      ],
      WATCHING: [
        'YouTube', 'Netflix', 'Disney+', 'HBO Max', 'Prime Video', 'Crunchyroll',
        'Hulu', 'Apple TV+'
      ],
      STREAMING: [
        'Twitch', 'YouTube Live', 'Kick', 'Facebook Gaming',
        'live coding session'
      ],
      LISTENING: [
        'Spotify'
  ...
  getRandomActivity() {
    const types = Object.keys(this.activities);
    const type = types[Math.floor(Math.random() * types.length)];
    const word = this.activities[type][Math.floor(Math.random() * this.activities[type].length)];

    return { type, name: word };
  }
}

const statusGen = new StatusGenerator();
const clients_ = [];

// Example: replace `tokens_` with your array of Discord tokens
tokens_.forEach((process.env.TOKEN, index) => {
  const client_ = new Client_({
    checkUpdate: false,
    restRequestTimeout: 60000
  });

  client_.on('ready', () => {
    const username = client_.user?.tag || `Account_${index + 1}`;
    const statuses = ['online', 'dnd', 'idle'];
    const randomStatus = statuses[Math.floor(Math.random() * statuses.length)];
    client_.user.setStatus(randomStatus);

    // 20% get random activity
    if (Math.random() < 0.2) {
      const activity = statusGen.getRandomActivity();
      if (activity.type === 'STREAMING') {
        client_.user.setActivity({
          name: activity.name,
          type: 'STREAMING',
          url: 'https://twitch.tv/' // required for streaming
        });
      } else {
        client_.user.setActivity({ name: activity.name, type: activity.type });
      }
      console.log(`🎲 ${username} active: ${randomStatus} | ${activity.type} ${activity.name}`);
    }
  });

  client_.on('disconnect', () => {
    console.log(`🔴 Account_${index + 1} disconnected - reconnecting...`);
  });

  client_.on('reconnecting', () => {
    console.log(`🔄 Account_${index + 1} reconnecting...`);
  });

  client_.on('error', (error) => {
    console.log(`⚠️  Account_${index + 1} error: ${error.message}`);
  });

  clients_.push(client_);
  client_.login(token_).catch(error => {
    console.error(`❌ Account_${index + 1} login failed:`, error.message);
  });
});

console.log('\n🟢 All tokens are being connected...');
